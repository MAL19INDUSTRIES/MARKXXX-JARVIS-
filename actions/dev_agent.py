...
Code for {file_path}:"""
    ...
    try:
        ...
        write_text_with_stamp(
            full_path,
            code,
            f"Generated by JARVIS dev_agent for project: {project_description}. File purpose: {file_desc}",
        )
...
Fixed code for {fix_path}:"""
        ...
        try:
            ...
            write_text_with_stamp(
                full_path,
                fixed,
                f"Auto-fixed by JARVIS dev_agent after build error for project: {project_description}",
            )
